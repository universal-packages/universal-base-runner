...
import { PrepareOnMultiMode, ReleaseOnMultiMode, RunMode, Status } from './BaseRunner.types'
...
class TestRunner extends BaseRunner {
  ...
  // Track prepare and release calls
...
export async function baseRunnerTest() {
  ...
  await runTest('BaseRunner should complete lifecycle quickly on timeout while internal run continues in background', async () => {
    ...
    assert(runner.measurement?.milliseconds! >= 95, `Runner should take at least close to the timeout duration (100ms), but took ${runner.measurement?.milliseconds}ms`)
  ...
  // Multi-run tests
  await runTest('BaseRunner should default to single run mode', async () => {
    const runner = new TestRunner()
    assertEquals(runner.options.runMode, RunMode.Single, 'Default run mode should be Single')
  })

  await runTest('BaseRunner should accept runMode option', async () => {
    const runner = new TestRunner({ runMode: RunMode.Multi })
    assertEquals(runner.options.runMode, RunMode.Multi, 'Run mode should be Multi when specified')
  })

  await runTest('BaseRunner in single mode should not allow multiple runs', async () => {
    const runner = new TestRunner({ runMode: RunMode.Single })

    await runner.run()
    assertEquals(runner.status, Status.Succeeded, 'First run should succeed')
    ...
    assert(errorThrown, 'Error should have been thrown on second run')
  })

  await runTest('BaseRunner in multi mode should allow multiple successful runs', async () => {
    const runner = new TestRunner({ runMode: RunMode.Multi })

    // First run
    await runner.run()
    assertEquals(runner.status, Status.Idle, 'Should reset to idle after first run')

    // Second run
    await runner.run()
    assertEquals(runner.status, Status.Idle, 'Should reset to idle after second run')

    // Third run
    await runner.run()
    assertEquals(runner.status, Status.Idle, 'Should reset to idle after third run')
  })

  await runTest('BaseRunner in multi mode should reset after failure', async () => {
    const runner = new TestRunner({ runMode: RunMode.Multi })
    runner.shouldReturnFailureReason = 'Test failure'

    await runner.run()
    assertEquals(runner.status, Status.Idle, 'Should reset to idle after failed run')

    // Should be able to run again
    runner.shouldReturnFailureReason = undefined
    await runner.run()
    assertEquals(runner.status, Status.Idle, 'Should reset to idle after successful run')
  })

  await runTest('BaseRunner in multi mode should reset after being stopped', async () => {
    const runner = new TestRunner({ runMode: RunMode.Multi })
    runner.runDelay = 100
    ...
    assertEquals(runner.status, Status.Idle, 'Should reset to idle after being stopped')
  ...
  await runTest('BaseRunner in multi mode should reset after skip', async () => {
    const runner = new TestRunner({ runMode: RunMode.Multi })

    await runner.skip('Test skip')
    assertEquals(runner.status, Status.Idle, 'Should reset to idle after skip')
  ...
  await runTest('BaseRunner in multi mode should reset after fail', async () => {
    const runner = new TestRunner({ runMode: RunMode.Multi })

    await runner.fail('Test fail')
    assertEquals(runner.status, Status.Idle, 'Should reset to idle after fail')
  ...
  await runTest('BaseRunner in multi mode should reset after timeout', async () => {
    const runner = new TestRunner({ runMode: RunMode.Multi, timeout: 50 })
    runner.runDelay = 100 // Will cause timeout

    await runner.run()
    assertEquals(runner.status, Status.Idle, 'Should reset to idle after timeout')
  ...
  await runTest('BaseRunner in multi mode should reset after error in prepare', async () => {
    const runner = new TestRunner({ runMode: RunMode.Multi })
    runner.shouldFailInPrepare = true

    await runner.run()
    assertEquals(runner.status, Status.Idle, 'Should reset to idle after prepare error')

    // Should be able to run again
    runner.shouldFailInPrepare = false
    await runner.run()
    assertEquals(runner.status, Status.Idle, 'Should reset to idle after successful run')
  })

  console.log('\n✅ All BaseRunner multi-run tests completed!')

  // ===========================================
  // PREPARE ON MULTI MODE TESTS
  // ===========================================
  console.log('\n🧪 Testing BaseRunner prepareOnMultiMode Options')
  console.log('='.repeat(50))

  await runTest('BaseRunner prepareOnMultiMode.Always should prepare on every run', async () => {
    const runner = new TestRunner({
      runMode: RunMode.Multi,
      prepareOnMultiMode: PrepareOnMultiMode.Always
    })

    // First run
    await runner.run()
    assertEquals(runner.prepareCalls, 1, 'Should have prepared on first run')
    assertEquals(runner.status, Status.Idle, 'Should reset to idle after first run')

    // Second run
    await runner.run()
    assertEquals(runner.prepareCalls, 2, 'Should have prepared on second run')
    assertEquals(runner.status, Status.Idle, 'Should reset to idle after second run')

    // Third run
    await runner.run()
    assertEquals(runner.prepareCalls, 3, 'Should have prepared on third run')
    assertEquals(runner.status, Status.Idle, 'Should reset to idle after third run')
  })

  await runTest('BaseRunner prepareOnMultiMode.Never should never prepare after first run', async () => {
    const runner = new TestRunner({
      runMode: RunMode.Multi,
      prepareOnMultiMode: PrepareOnMultiMode.Never
    })

    // First run
    await runner.run()
    assertEquals(runner.prepareCalls, 0, 'Should not have prepared on first run')
    assertEquals(runner.status, Status.Idle, 'Should reset to idle after first run')

    // Second run
    await runner.run()
    assertEquals(runner.prepareCalls, 0, 'Should not have prepared on second run')
    assertEquals(runner.status, Status.Idle, 'Should reset to idle after second run')

    // Third run
    await runner.run()
    assertEquals(runner.prepareCalls, 0, 'Should not have prepared on third run')
    assertEquals(runner.status, Status.Idle, 'Should reset to idle after third run')
  })

  await runTest('BaseRunner prepareOnMultiMode.OnFirstRun should only prepare on first run', async () => {
    const runner = new TestRunner({
      runMode: RunMode.Multi,
    ...
    // Second run
    await runner.run()
    assertEquals(runner.prepareCalls, 1, 'Should not have prepared again on second run')
    assertEquals(runner.status, Status.Idle, 'Should reset to idle after second run')

    // Third run
    await runner.run()
    assertEquals(runner.prepareCalls, 1, 'Should not have prepared again on third run')
    assertEquals(runner.status, Status.Idle, 'Should reset to idle after third run')
  })

  await runTest('BaseRunner should default to prepareOnMultiMode.Always in multi mode', async () => {
    const runner = new TestRunner({ runMode: RunMode.Multi })

    // First run
    await runner.run()
    assertEquals(runner.prepareCalls, 1, 'Should have prepared on first run (default behavior)')

    // Second run
    await runner.run()
    assertEquals(runner.prepareCalls, 2, 'Should have prepared on second run (default behavior)')
  })

  // ===========================================
  // RELEASE ON MULTI MODE TESTS
  // ===========================================
  console.log('\n🧪 Testing BaseRunner releaseOnMultiMode Options')
  console.log('='.repeat(50))

  await runTest('BaseRunner releaseOnMultiMode.Always should release on every run', async () => {
    const runner = new TestRunner({
      runMode: RunMode.Multi,
      releaseOnMultiMode: ReleaseOnMultiMode.Always
    })

    // First run
    await runner.run()
    assertEquals(runner.releaseCalls, 1, 'Should have released on first run')
    assertEquals(runner.status, Status.Idle, 'Should reset to idle after first run')

    // Second run
    await runner.run()
    assertEquals(runner.releaseCalls, 2, 'Should have released on second run')
    assertEquals(runner.status, Status.Idle, 'Should reset to idle after second run')

    // Third run
    await runner.run()
    assertEquals(runner.releaseCalls, 3, 'Should have released on third run')
    assertEquals(runner.status, Status.Idle, 'Should reset to idle after third run')
  })

  await runTest('BaseRunner releaseOnMultiMode.Never should never release', async () => {
    const runner = new TestRunner({
      runMode: RunMode.Multi,
    ...
    // First run
    await runner.run()
    assertEquals(runner.releaseCalls, 0, 'Should not have released on first run')
    assertEquals(runner.status, Status.Idle, 'Should reset to idle after first run')

    // Second run
    await runner.run()
    assertEquals(runner.releaseCalls, 0, 'Should not have released on second run')
    assertEquals(runner.status, Status.Idle, 'Should reset to idle after second run')

    // Third run
    await runner.run()
    assertEquals(runner.releaseCalls, 0, 'Should not have released on third run')
    assertEquals(runner.status, Status.Idle, 'Should reset to idle after third run')
  })

  await runTest('BaseRunner should default to releaseOnMultiMode.Never in multi mode', async () => {
    const runner = new TestRunner({ runMode: RunMode.Multi })

    // First run
    await runner.run()
    assertEquals(runner.releaseCalls, 0, 'Should not have released on first run (default behavior)')

    // Second run
    await runner.run()
    assertEquals(runner.releaseCalls, 0, 'Should not have released on second run (default behavior)')
  })

  // ===========================================
  // COMBINED PREPARE/RELEASE MODE TESTS
  // ===========================================
  console.log('\n🧪 Testing BaseRunner Combined prepareOnMultiMode/releaseOnMultiMode Options')
  console.log('='.repeat(50))

  await runTest('BaseRunner with prepareOnMultiMode.Always and releaseOnMultiMode.Always', async () => {
    const runner = new TestRunner({
      runMode: RunMode.Multi,
      prepareOnMultiMode: PrepareOnMultiMode.Always,
      releaseOnMultiMode: ReleaseOnMultiMode.Always
  ...
  await runTest('BaseRunner with prepareOnMultiMode.OnFirstRun and releaseOnMultiMode.Always', async () => {
    const runner = new TestRunner({
      runMode: RunMode.Multi,
      prepareOnMultiMode: PrepareOnMultiMode.OnFirstRun,
      releaseOnMultiMode: ReleaseOnMultiMode.Always
  ...
  await runTest('BaseRunner with prepareOnMultiMode.Never and releaseOnMultiMode.Never', async () => {
    const runner = new TestRunner({
      runMode: RunMode.Multi,
      prepareOnMultiMode: PrepareOnMultiMode.Never,
      releaseOnMultiMode: ReleaseOnMultiMode.Never
  ...
  await runTest('BaseRunner prepareOnMultiMode options should not affect single mode', async () => {
    const runner = new TestRunner({
      runMode: RunMode.Single,
      prepareOnMultiMode: PrepareOnMultiMode.Never // Should be ignored
    })

    await runner.run()
    assertEquals(runner.prepareCalls, 1, 'Should have prepared in single mode regardless of prepareOnMultiMode')
    assertEquals(runner.releaseCalls, 1, 'Should have released in single mode regardless of releaseOnMultiMode')
    assertEquals(runner.status, Status.Succeeded, 'Should be succeeded, not idle')
  })

  await runTest('BaseRunner releaseOnMultiMode options should not affect single mode', async () => {
    const runner = new TestRunner({
      runMode: RunMode.Single,
      releaseOnMultiMode: ReleaseOnMultiMode.Never // Should be ignored
    })
  ...
  // ===========================================
  // PREPARE/RELEASE MODE WITH FAILURES TESTS
  // ===========================================
  console.log('\n🧪 Testing BaseRunner prepareOnMultiMode/releaseOnMultiMode with Failures')
  console.log('='.repeat(50))

  await runTest('BaseRunner prepareOnMultiMode.OnFirstRun should still prepare again after failure', async () => {
    const runner = new TestRunner({
      runMode: RunMode.Multi,
      prepareOnMultiMode: PrepareOnMultiMode.OnFirstRun
    })
    ...
    runner.shouldReturnFailureReason = undefined
    await runner.run()
    assertEquals(runner.prepareCalls, 1, 'Should not prepare again after failure - still counts as "first run"')
    assertEquals(runner.status, Status.Idle, 'Should reset to idle after successful run')
  })

  await runTest('BaseRunner prepareOnMultiMode.OnFirstRun should reset prepare tracking after stop', async () => {
    const runner = new TestRunner({
      runMode: RunMode.Multi,
      prepareOnMultiMode: PrepareOnMultiMode.OnFirstRun
    })
    ...
    await runPromise
    assertEquals(runner.prepareCalls, 1, 'Should have prepared on first run')
    assertEquals(runner.status, Status.Idle, 'Should reset to idle after stopped run')

    // Second run - should not prepare again
    runner.runDelay = 0
    await runner.run()
    assertEquals(runner.prepareCalls, 1, 'Should not prepare again after stop - still counts as having done first run')
    assertEquals(runner.status, Status.Idle, 'Should reset to idle after successful run')
  })

  await runTest('BaseRunner prepareOnMultiMode.OnFirstRun should still prepare again after skip', async () => {
    const runner = new TestRunner({
      runMode: RunMode.Multi,
      prepareOnMultiMode: PrepareOnMultiMode.OnFirstRun
    })

    // First action - skip (no run)
    await runner.skip('Test skip')
    assertEquals(runner.prepareCalls, 0, 'Should not have prepared when skipped')
    assertEquals(runner.status, Status.Idle, 'Should reset to idle after skip')

    // Second action - actual run (should be treated as first run)
    await runner.run()
    assertEquals(runner.prepareCalls, 1, 'Should prepare on first actual run after skip')
    assertEquals(runner.status, Status.Idle, 'Should reset to idle after successful run')

    // Third action - second run (should not prepare)
    await runner.run()
    assertEquals(runner.prepareCalls, 1, 'Should not prepare again on second actual run')
    assertEquals(runner.status, Status.Idle, 'Should reset to idle after successful run')
  })

  await runTest('BaseRunner releaseOnMultiMode.Always should release even after failures', async () => {
    const runner = new TestRunner({
      runMode: RunMode.Multi,
      releaseOnMultiMode: ReleaseOnMultiMode.Always
    })
    ...
    runner.shouldReturnFailureReason = 'Test failure'
    await runner.run()
    assertEquals(runner.releaseCalls, 1, 'Should have released even after failure')
    assertEquals(runner.status, Status.Idle, 'Should reset to idle after failed run')

    // Second run - succeed
    runner.shouldReturnFailureReason = undefined
    await runner.run()
    assertEquals(runner.releaseCalls, 2, 'Should have released on successful run')
    assertEquals(runner.status, Status.Idle, 'Should reset to idle after successful run')
  })

  await runTest('BaseRunner releaseOnMultiMode.Always should release even after stop', async () => {
    const runner = new TestRunner({
      runMode: RunMode.Multi,
      releaseOnMultiMode: ReleaseOnMultiMode.Always
    })
    ...
    setTimeout(() => runner.stop('Test stop'), 50)
    await runPromise
    assertEquals(runner.releaseCalls, 1, 'Should have released even after being stopped')
    assertEquals(runner.status, Status.Idle, 'Should reset to idle after stopped run')
  ...
  console.log('\n✅ All BaseRunner prepareOnMultiMode/releaseOnMultiMode tests completed!')
}
